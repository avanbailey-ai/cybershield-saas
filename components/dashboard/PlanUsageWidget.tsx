...
export default function PlanUsageWidget() {
  const { plan, limits, websiteCount, scansToday, websitesRemaining, scansRemaining, loading } = usePlan();
  ...
  return (
    <div className="hidden items-center gap-3 rounded-lg border border-gray-800 bg-gray-900/50 px-3 py-1.5 lg:flex">
      ...
      {(websitesRemaining === 0 || scansRemaining === 0) && (
        <Link href="/dashboard/settings" className="text-xs font-medium text-orange-400 hover:text-orange-300">
          Upgrade
        </Link>
      )}
    </div>
