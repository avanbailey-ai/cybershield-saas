...
export default function ScanResultPaywall({
  ...
  function handlePricingClick(trigger: string) {
  ...
  return (
    <div className="mt-8 rounded-xl border border-gray-700/60 bg-gray-900/60 p-5 text-left sm:p-6">
      ...
      {requireExplicitClick && !showPaywall && lockedCount > 0 && (
        <div className="mt-6 rounded-lg border border-gray-700/50 bg-gray-800/30 p-5 text-center">
          ...
          <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            ...
            >
              Fix issues automatically (Pro upgrade)
            </button>
          </div>
        </div>
      )}
      ...
      {lockedCount > 0 && paywallVisible && (
        <div className="relative mt-6 overflow-hidden rounded-lg border border-gray-700/50">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-950/75 px-4 py-6">
            ...
            {funnel.showEnterpriseCta ? (
              <Link
                href={funnel.smbPrimaryHref}
                onClick={() => handlePricingClick('locked_blur')}
                className="w-full max-w-xs rounded-lg bg-blue-600 px-5 py-3 text-center text-sm font-semibold text-white transition-colors hover:bg-blue-500"
              >
                {funnel.smbPrimaryCta}
              </Link>
            ) : (
              <div className="flex w-full max-w-md flex-col gap-2">
                <button
                  ...
                  className="w-full rounded-lg bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-blue-500"
                >
                  Fix issues automatically (Pro upgrade)
                </button>
                <button
                  type="button"
                  onClick={() =>
                    trackEvent('report_viewed', { domain: result.url, trigger: 'full_report_unlock' })
                  }
                  className="w-full rounded-lg border border-gray-600 px-5 py-2.5 text-sm font-medium text-gray-300 hover:border-gray-500 hover:text-white"
                >
                  View full report
                </button>
              </div>
      ...
      {paywallVisible && lockedCount === 0 && (
        <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row">
          {funnel.showEnterpriseCta ? (
            <Link
              href={funnel.smbPrimaryHref}
              onClick={() => handlePricingClick('primary_cta')}
              className="w-full rounded-lg bg-blue-600 px-4 py-3 text-center text-sm font-semibold text-white transition-colors hover:bg-blue-500 sm:flex-1"
            >
              {funnel.smbPrimaryCta}
            </Link>
          ) : (
            <>
              ...
              <button
                type="button"
                onClick={() =>
                  trackEvent('report_viewed', { domain: result.url, trigger: 'view_full_report' })
                }
                className="w-full rounded-lg border border-gray-600 px-4 py-3 text-sm font-medium text-gray-300 hover:border-gray-500 hover:text-white sm:flex-1"
              >
                View full report
              </button>
            </>
      ...
      {paywallVisible && lockedCount > 0 && funnel.showEnterpriseCta && (
        <div className="mt-4 flex justify-center">
          <Link
            href={funnel.smbPrimaryHref}
            onClick={() => handlePricingClick('secondary_smb')}
      ...
      {paywallVisible && funnel.showEnterpriseCta && (
        <div className="mt-8 rounded-lg border border-gray-700/50 bg-gray-800/20 p-4">
          ...
          <p className="mt-1 text-xs text-gray-400">
            Enterprise plans include compliance reporting, continuous monitoring, audit trails, and
