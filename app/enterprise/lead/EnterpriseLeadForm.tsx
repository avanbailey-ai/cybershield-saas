...
const SECURITY_NEED_GROUPS = [
  ...
  },
  {
    ...
    needs: ['Multi-site Continuous Monitoring', 'Penetration Testing', 'API Security'],
  },
] as const;
        ...
        <main className="mx-auto max-w-xl px-4 py-16 text-center">
          <div className="rounded-xl border border-blue-500/30 bg-blue-500/10 p-8">
            ...
            <p className="mt-6 text-xs text-gray-500">
              Enterprise requests are reviewed by email. No call is scheduled from this form.
            </p>
          </div>
        </main>
      </div>
      ...
      <main className="mx-auto max-w-2xl px-4 py-12">
        ...
        {(fromScan || isReview) && !needsEnterpriseAccess && (
          <div className="mb-6 rounded-xl border border-amber-500/30 bg-amber-500/10 p-5 text-center">
            <h2 className="text-xl font-semibold text-white">
              Enterprise review may be appropriate
              {scanDomain ? ` for ${scanDomain}` : ''}
            </h2>
            <p className="mt-2 text-sm text-amber-200/80">
              {scanScore != null
                ? `Your site scored ${scanScore}/100. If this site has compliance, SSO, audit-log, or custom SLA requirements, request an enterprise review.`
                : 'If this site has compliance, SSO, audit-log, or custom SLA requirements, request an enterprise review.'}
            </p>
          </div>
        )}
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-gray-400">
            For organizations that need SSO/SAML, audit logs, custom SLA, compliance reporting, or
            multi-site governance. If you only need self-serve monitoring, compare Pro, Growth, and
            Agency plans instead.
          </p>
          <Link href="/pricing" className="mt-3 inline-flex text-sm font-medium text-blue-400 hover:text-blue-300">
            Compare self-serve monitoring plans →
          </Link>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-gray-800 bg-gray-900/50 p-6">
          ...
          >
            {loading ? 'Submitting...' : 'Submit enterprise review request'}
          </button>

          <p className="text-center text-xs text-gray-500">
            Best fit for 200+ employees, regulated teams, or organizations requiring SSO, audit
            logs, custom SLA, or dedicated review.
          </p>
        </form>
