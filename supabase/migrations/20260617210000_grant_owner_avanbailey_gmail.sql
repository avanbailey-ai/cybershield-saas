-- Grant agency testing entitlement to platform owner account (avanbailey@gmail.com).
-- Safe to run if user already has entitlement; resolves org/subscription for Founder OS access.
...
BEGIN
  ...
  FROM auth.users u
  WHERE lower(u.email) = lower('avanbailey@gmail.com')
  LIMIT 1;

  IF v_user_id IS NULL THEN
    RAISE NOTICE 'Owner user avanbailey@gmail.com not found — sign up first, then re-run migration';
    RETURN;
  END IF;
