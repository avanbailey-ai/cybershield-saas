/**
 * Recompute stored prospect pipeline verdicts (ICP, queue, contact, agency classification).
 *
 * Safe: no deletes, no emails, no draft sends, no contact fabrication.
 *
 * Usage:
 *   npx tsx scripts/backfill-prospect-pipeline-verdicts.ts --dry-run
 *   npx tsx scripts/backfill-prospect-pipeline-verdicts.ts --apply
 *   npx tsx scripts/backfill-prospect-pipeline-verdicts.ts --input=prospects.json --sql-out=updates.sql
 */
