...
async function checkEnterprisePortal() {
  ...
  if (location) {
    ...
    const smokeUrl = new URL(`${BASE_URL}/enterprise/portal`);

    if (redirectUrl.pathname === smokeUrl.pathname && redirectUrl.origin !== smokeUrl.origin) {
      res = await request(redirectUrl.toString(), { redirect: 'manual' });
      if (res.status === 500) {
        fail(name, `GET ${redirectUrl.toString()} returned 500`);
      }
      if (!redirectStatuses.has(res.status)) {
        fail(name, `canonical enterprise portal expected redirect, got ${res.status}`);
      }
