...
assert(filters.includes('isInternalCustomerProfile'), 'Internal customer profile filter exists');
assert(filters.includes('OWNER_EMAIL'), 'Owner email excluded from metrics');
assert(legacyFilters.includes("from './internalAccountFilters'"), 'Deprecated customer filter module re-exports current filters');
