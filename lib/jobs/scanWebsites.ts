...
import { resolveMonitoringScanKind } from './monitoringScanKind';
...
export async function runScheduledScans(): Promise<ScheduledScanResult> {
  ...
  const { data: websites, error } = await supabase
    ...
    .select(
      ...
      'id, url, user_id, org_id, last_scanned_at, last_deep_scan_at, next_scan_at, scan_frequency, is_active, priority_monitoring',
  ...
  if (error || !websites) {
    ...
    return { examined: 0, due: 0, queued: 0, skipped: 0, blocked: 0, errors: 0 };
  ...
  for (const website of websites) {
    ...
    if (plan === 'free') {
      ...
      logSchedulerDecision({
        ...
        reason: 'free_no_recurring_monitoring',
    ...
    const monitoringScanKind = resolveMonitoringScanKind(mode!, {

      scanFrequency: website.scan_frequency,

      priorityMonitoring,

      lastDeepScanAt: website.last_deep_scan_at,
  ...
  return { examined: websites.length, due, queued, skipped, blocked, errors };
